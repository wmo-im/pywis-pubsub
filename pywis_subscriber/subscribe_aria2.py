...
if args.config == "":
    config_filename = "../pywis-pubsub-config.yml"
else:
...
def write_toDownload(msg_id, url,
    ...
    if download_orgFilename is True:
        myURL = urlparse(url)
...
with open(config_filename, 'r') as myConfigFile:
    myConfig = yaml.safe_load(myConfigFile)
...
if "sub_logfile" in myConfig.keys():
    ...
    sub_logfile = "subscribe_" + printTimeNow + ".log"
...
if sub_protocol == "mqtts" or sub_protocol == "mqtt":
    ...
    print("error - this script is only for mqtt(s).")
...
sub_brokerAddress = sub_protocol + "://" + sub_user + ":" + sub_password + "@" + sub_host + ":" + str(sub_port)
message_broker = sub_protocol + "://" + sub_user + ":[passwd]@" + sub_host + ":" + str(sub_port)
...
if "sub_share_name" in myConfig.keys():
    ...
    sub_share_name = "mySubGroupName_pleaseChange_" + str(uuid.uuid1())
...
if "sub_validate" in myConfig.keys():
    sub_validate = myConfig["sub_validate"]
else:
    sub_validate = False
...
if sub_validate is True:
    schema = json.load(open("WIS2_Message_Format_Schema.json"))

# write log for config file values
if toSubscribe is False:
